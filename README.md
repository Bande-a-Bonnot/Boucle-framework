...
Claude Code hooks that actually enforce your rules. 9 hooks, ~1758 tests, [898 known Claude Code gaps documented](tools/enforce/#known-limitations) with severity ratings and workarounds.
...
New in v0.12.0: Searchable Known Limitations page (now 898 entries). test-hook.sh dry-run tester. file-guard v2.1.89 absolute path fix. safety-check 3 silent failure fixes. Installer UX improvements. README navigation banner + platform matrix. See [CHANGELOG](CHANGELOG.md) for details.
