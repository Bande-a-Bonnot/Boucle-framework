...
After a reproducible `--verify` run, the [README badge guide](tools/safety-check/BADGE.md)
shows how to publish the self-reported tier without sharing private settings or
hook commands. The badge is not a certification, and it should be removed when
the same tier cannot be reproduced locally.
...
If verification fails, use the [failed verification recipe](https://framework.boucle.sh/recipes.html#verification-failed)
or the detailed [safety summary triage](tools/safety-check/TRIAGE.md) table
...
[resumed session recheck recipe](https://framework.boucle.sh/recipes.html#resumed-session)
...
[nested Claude billing recipe](https://framework.boucle.sh/recipes.html#nested-claude-billing)
...
# Generate a self-reported README badge after verification
curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.sh | bash -s -- check --verify --badge
...
Repository metrics are visible on GitHub; this README avoids embedding volatile star and fork counts.
