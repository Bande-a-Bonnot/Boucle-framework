...
Documented limitations of Claude Code's hook system, collected from GitHub issues and testing. [Searchable version](https://framework.boucle.sh/limitations.html) with filtering by category and issue number. Large clusters include:

| Category | Examples |
|----------|----------|
| Hook behavior & events | Async stdin empty, exit code handling, slash command bypass, no user-prompt event, hooks stop after long sessions, PostToolUse format-on-save breaks consecutive edits, CLAUDE_ENV_FILE broken, no persistent child env, statusline receives hook stdout, no ToolStarted event between PreToolUse and PostToolUse |
| Permission system | MCP deny ignored, path matching, self-authorization race, scope hierarchy, deny rules don't protect CLAUDE.md, 50-subcommand deny bypass, DenyRead/Write overridden by user AllowRead/Write, `git restore` without confirmation |
| Hook bypass & evasion | @-autocomplete, pipe mode, `--bare`, subdirectory launch skipping project hooks, subagent `omitClaudeMd`, Edit->Bash tool switch, `$()` subshell pattern-match failure, goal-directed tool switching, apiKeyHelper arbitrary code execution, `find` command injection (CVE-2026-24887) |
| MCP & plugin issues | OAuth/auth fallback bugs, missing tool injection, channel/plugin lifecycle failures, stale marketplace state, connected-but-unusable servers |
| Subagent & spawned agents | Settings not inherited, deny rules bypassed, no CLAUDE.md loaded, teammate hooks bypass, subagent file creation bypass, worktree isolation wrong commit |
...
**Windows: hooks run via `/usr/bin/bash` regardless of shell setting.** On Windows, Claude Code [routes all hook commands through `/usr/bin/bash`](https://github.com/anthropics/claude-code/issues/32930) even when a different shell is configured. Bash-based hooks work if Git Bash is installed (it provides `/usr/bin/bash`). All 7 standalone Boucle hooks now ship native PowerShell equivalents (`.ps1`) that bypass this limitation. Use `pwsh -File path/to/hook.ps1` in your hook command to run them directly. See [install.ps1](../install.ps1) for one-line setup.

**Hook ask/deny is not enforced for MCP tool calls.** PreToolUse hooks can appear to match MCP server tools in direct stdin tests, but [`permissionDecision: "deny"`](https://github.com/anthropics/claude-code/issues/33106) and [`permissionDecision: "ask"`](https://github.com/anthropics/claude-code/issues/81569) have both been reported as ignored during real MCP tool execution. The MCP call proceeds without the expected block or prompt. This is a platform bug, not an enforce-hooks limitation. Workaround: use MCP server-side access controls, remove untrusted MCP servers, or block the MCP server name in managed-settings.json `disallowedTools` where available.
...
**Hook input lacks reliable agent context for tool calls.** Hook payloads do not provide a reliable per-agent identity for tool calls. Older reports found that `agent_id` and `agent_type` were only available in `SubagentStart`/`SubagentStop` hook events and [absent from PreToolUse and PostToolUse input](https://github.com/anthropics/claude-code/issues/40140), so hooks could not tell whether a tool call came from the main conversation or a subagent. A newer Claude Code 2.1.251 report found the opposite-looking but equally unsafe shape: some `PreToolUse`/`PostToolUse` calls from real running subagents were [attributed to a different `agent_id`](https://github.com/anthropics/claude-code/issues/90662) that had no `SubagentStart`, no `agent_type`, and no later `SubagentStop`. Do not enforce per-agent policy or close audit spans from `PreToolUse`/`PostToolUse` `agent_id` alone until the installed Claude Code version proves lifecycle consistency. Reconcile every tool-call `agent_id` against a `SubagentStart`/`SubagentStop` roster, treat unknown or unclosed ids as tainted telemetry, and rely on tool-restricted agent definitions, filesystem or git controls, and post-run verification for load-bearing subagent policy.
...
**`CLAUDE_CODE_SIMPLE` mode disables all hooks.** When the `CLAUDE_CODE_SIMPLE` environment variable is set to any non-empty value, Claude Code [disables hooks, MCP tools, attachments, and CLAUDE.md file loading entirely](https://github.com/anthropics/claude-code/blob/main/CHANGELOG.md) (v2.1.50). Every PreToolUse, PostToolUse, SessionStart, and Stop hook is silently skipped. CLAUDE.md rules are not loaded. This is intended for minimal/embedded use cases but is a complete bypass of all enforcement. Not hookable, since hooks themselves are what is disabled. safety-check warns when `CLAUDE_CODE_SIMPLE` is detected in the environment. Related: `IS_DEMO=1` has the same effect via a different mechanism ([#37780](https://github.com/anthropics/claude-code/issues/37780)).
...
**`WorktreeCreate` hooks cause indefinite session hang.** Any `WorktreeCreate` hook configured in project settings causes [`claude -w` to hang forever](https://github.com/anthropics/claude-code/issues/41614). Even a trivial hook (`echo ok < /dev/null`) causes the session to freeze. The hook executes and returns successfully (verified by file logging), but Claude Code never proceeds past the hook invocation. This is distinct from the [EnterWorktree ignoring hooks](#known-limitations) issue ([#36205](https://github.com/anthropics/claude-code/issues/36205)) — here the hook fires but the response is never consumed. Remove all `WorktreeCreate` hooks if you need `claude -w` to function. See [#41614](https://github.com/anthropics/claude-code/issues/41614), [#42752](https://github.com/anthropics/claude-code/issues/42752).
...
**Custom commands and skills broken in v2.1.88-89.** Custom slash commands from `.claude/commands/` [do not appear in autocomplete and return "Unknown skill"](https://github.com/anthropics/claude-code/issues/41864) when invoked via the Skill tool in v2.1.89. A related regression in v2.1.88 causes [skills to invoke the wrong one or fail entirely](https://github.com/anthropics/claude-code/issues/41882), due to an `EACCES` error on the bundled ripgrep binary. Anthropic marked that command-discovery regression fixed in v2.1.91. Separately, standalone `.md` files in `.claude/skills/` are [not discoverable via slash command search](https://github.com/anthropics/claude-code/issues/41855); only the `folder/SKILL.md` pattern works. These regressions affect workflows that deliver enforcement rules or operational procedures via custom skills. Update to the latest Claude Code release and verify slash commands inside a fresh session. See [#41864](https://github.com/anthropics/claude-code/issues/41864), [#41882](https://github.com/anthropics/claude-code/issues/41882), [#41855](https://github.com/anthropics/claude-code/issues/41855). Related to [#41530](https://github.com/anthropics/claude-code/issues/41530) (v2.1.88 skills regression).
...
**PostToolUse format-on-save hooks break consecutive edits (fixed v2.1.90).** When a PostToolUse hook reformats a file after an Edit or Write (e.g., running prettier, black, or gofmt), the next Edit/Write to the same file fails with "File content has changed." The formatter changes the file hash between tool calls, and Claude Code's staleness check rejects the subsequent edit. Fixed in v2.1.90 -- the platform now accounts for PostToolUse modifications to file content. On pre-v2.1.90, workaround: use a separate formatting step instead of a PostToolUse hook, or accept that multi-edit sequences to the same file will require retries.

**PowerShell trailing `&` bypasses tool permission checks (fixed v2.1.90).** Appending `&` to a PowerShell command launched it as a background job, bypassing the tool permission evaluation. A denied command like `Remove-Item -Recurse` could execute unchecked as `Remove-Item -Recurse &`. Fixed in v2.1.90 alongside three other PowerShell hardening fixes: `-ErrorAction Break` causing a debugger hang, archive-extraction TOCTOU (time-of-check-time-of-use race), and parse-failure fallback degrading deny rules. On pre-v2.1.90, PowerShell permission checks have multiple bypass vectors.
...
**Model switches tools to bypass denied operations (goal-directed evasion).** When a deny rule blocks a specific command (e.g., `Bash(rm *)`), the model [uses alternative tools](https://github.com/anthropics/claude-code/issues/39459) to accomplish the same goal: `python3 -c "import os; os.remove('file')"` when `rm` is denied, or Node.js `fs.unlinkSync()`, Ruby `File.delete()`, Perl `unlink()`. The model treats permission blocks as "tool blocked" not "goal blocked" and pivots to equivalent operations in other languages. Deny rules and hooks that match specific commands are fundamentally limited by this: blocking `rm` requires also blocking every other deletion vector. Mitigation: write hooks that match the *goal* (file deletion, network access) across all interpreters, not just the specific binary. See [#39459](https://github.com/anthropics/claude-code/issues/39459). Related to [#38841](https://github.com/anthropics/claude-code/issues/38841) (state file manipulation) and [#40117](https://github.com/anthropics/claude-code/issues/40117) (--no-verify bypass with multiple tactics).

**`bypassPermissions` does not suppress multi-line Bash description safety check.** The `--dangerously-skip-permissions` flag and `bypassPermissions` permission mode do not suppress Claude Code's built-in multi-line Bash command safety check. Users in bypass mode [still get prompted](https://github.com/anthropics/claude-code/issues/39875) with a confirmation dialog when commands contain newlines. This breaks automated workflows and headless `-p` scripts that expect bypass mode to suppress all prompts. The safety check fires independently of the permission system. Labeled as duplicate but the behavior persists. See [#39875](https://github.com/anthropics/claude-code/issues/39875).
...
**--resume loads 0% context on v2.1.91: three regressions in session loading pipeline.** In v2.1.91, `--resume` and `/resume` [silently load 0% of conversation history](https://github.com/anthropics/claude-code/issues/43044). Three regressions compound: (1) the new synchronous reader skips fork pruning for files >5 MB, (2) a new timestamp fallback bridges across fork boundaries connecting messages from different forks, (3) `getLastSessionLog` uses `findLatestMessage` without `leafUuids` check, picking synthetic messages from resume attempts. Each failed resume further corrupts the session JSONL by forking the parentUuid DAG. Labeled data-loss. Reporter documented $364 in extra API costs from broken resume plus cache invalidation in a single day. See [#43044](https://github.com/anthropics/claude-code/issues/43044).

**DenyRead/Write overridden by user AllowRead/Write on matching path.** When an admin sets `denyRead` or `denyWrite` on a path in managed settings, a user can [override it by adding the same path to `allowRead` or `allowWrite`](https://github.com/anthropics/claude-code/issues/43043) in their own settings.json. The allow rule wins over the deny rule on exact path matches regardless of settings scope. `allowManagedReadPathsOnly` exists as a workaround but adds friction. Reporter notes this never worked (not a regression). Labeled as security issue. See [#43043](https://github.com/anthropics/claude-code/issues/43043).

**Statusline command receives hook stdout instead of structured JSON metadata.** The statusline command (configured in settings.json for HUD display) [occasionally receives raw hook stdout](https://github.com/anthropics/claude-code/issues/43035) instead of Claude Code's structured JSON metadata (model, context_window, rate_limits). Hook output is incorrectly routed to the statusline command's stdin instead of being routed exclusively to the hook result parser. Causes raw JSON or truncated text in the statusline display. Affects v2.1.91 when using statusline with hooks that return output. See [#43035](https://github.com/anthropics/claude-code/issues/43035).

**`autoAllowBashIfSandboxed: true` does not auto-approve commands with shell variable expansions.** When `autoAllowBashIfSandboxed: true` is set and sandboxing is enabled, commands containing shell variable expansions (`$VAR`), backtick substitution, or other non-literal constructs [still trigger permission prompts](https://github.com/anthropics/claude-code/issues/43713) with reasons like "Contains simple_expansion" or "Unhandled node type: string". Only syntactically simple commands (literal argv, pipes, `&&`, `;`, `||`, `[[ ]]`, `<<<`, control flow) are auto-approved. Shell expansion coverage is incomplete, defeating the purpose of the setting for scripts that use variables. Workaround: restructure commands to avoid variable expansion, or add explicit allow patterns for affected command forms. See [#43713](https://github.com/anthropics/claude-code/issues/43713).

**Background teammate `SendMessage` to team-lead is silently dropped (one-way only).** When a teammate is spawned via `Agent()` with `run_in_background: true` and `team_name`, `SendMessage` works in one direction only. Team-lead to teammate messages are delivered correctly. Teammate to team-lead messages ([`SendMessage({ to: "team-lead" })`](https://github.com/anthropics/claude-code/issues/43706)) return success on the teammate's end but never appear in the team-lead's conversation, silently dropped with no error. This breaks bidirectional coordination patterns in multi-agent teams. Workaround: use shared files for teammate-to-lead communication, or use foreground agents instead. See [#43706](https://github.com/anthropics/claude-code/issues/43706).

**Opus 4.6 (1M context) silently ignores CLAUDE.md rules and user instructions in long sessions.** During extended sessions with Claude Opus 4.6 using the 1M context window, the model can [repeatedly ignore CLAUDE.md rules and explicit user instructions](https://github.com/anthropics/claude-code/issues/43716) even after they were loaded at session start. Instructions repeated 5+ times by the user may still be disregarded. The behavior persists across `/clear` resets, ruling out context length as the sole cause. Affects agentic workflows relying on persistent behavioral constraints. Workaround: inject critical instructions inline with each task prompt rather than relying on CLAUDE.md alone; use shorter sessions or smaller context window models for strict rule-following. See [#43716](https://github.com/anthropics/claude-code/issues/43716).

**Agent triggers "violative cyber content" refusal on security-related filenames in local directories.** Automated Claude Code agents refuse to proceed with tasks when the local repository [contains filenames associated with security research](https://github.com/anthropics/claude-code/issues/43703) (e.g., `malware.py`, `exploit.sh`, CTF challenge files). The refusal cites "violative cyber content" and is triggered by filename patterns rather than actual file contents. Legitimate security research, penetration testing tooling, and CTF repositories are blocked. Not hookable. Workaround: rename files to avoid triggering content filter patterns before running agents, or explicitly inform the agent the repository contains legitimate security research. See [#43703](https://github.com/anthropics/claude-code/issues/43703).

**Cowork auto-update reformats the session disk, destroying all user projects.** The Cowork daemon [reformats the session disk on auto-update](https://github.com/anthropics/claude-code/issues/43719) instead of mounting the existing filesystem after sdk-daemon updates. All user projects on the session disk are destroyed without warning. This is a critical data-loss issue for any workflow that relies on persistent session state surviving Cowork updates. Workaround: disable auto-update for Cowork, or manually back up the session disk before updates. See [#43719](https://github.com/anthropics/claude-code/issues/43719).

**Dispatch tasks hang indefinitely on Desktop/Windows.** Tasks dispatched via the Dispatch tool [reach the Desktop app on Windows but hang indefinitely on "thinking"](https://github.com/anthropics/claude-code/issues/43726) and never produce a response. No timeout, error, or hook event is fired. Hook-based enforcement around dispatch completion is unreachable on this platform. Workaround: use the CLI instead of the Desktop app on Windows for all dispatch tasks. See [#43726](https://github.com/anthropics/claude-code/issues/43726).

**Worktrees accumulate in `.claude/worktrees/` with no automatic cleanup.** Worktrees created by the Agent tool's `isolation: "worktree"` option [accumulate in `.claude/worktrees/`](https://github.com/anthropics/claude-code/issues/43730) with no automatic cleanup. Sessions that spawn many isolated agents leave dozens of stale worktrees consuming disk space, and stale worktrees can cause path resolution errors in subsequent agents. Workaround: periodically run `git worktree prune` or `git worktree remove` to clean up stale entries. See [#43730](https://github.com/anthropics/claude-code/issues/43730).

**Plugin update command reports "already latest" without fetching the remote registry.** The plugin update command [reads a stale local cache](https://github.com/anthropics/claude-code/issues/43732) and reports the plugin is already up to date without fetching the remote registry. Users who push updates to custom marketplace repos will not see updates propagated until the cache expires or is manually cleared. Hooks and enforcement rules shipped in plugin updates are silently not applied. Workaround: manually delete `~/.claude/plugins/marketplaces/<name>/` to force a fresh fetch. See [#43732](https://github.com/anthropics/claude-code/issues/43732).

**JetBrains IDEA extension silently overwrites in-progress edits with stale cached versions.** The JetBrains IDEA extension for Claude Code [silently overwrites in-progress file edits with stale versions from cache](https://github.com/anthropics/claude-code/issues/43734). No warning or conflict dialog is shown. This creates a data-loss risk for users who edit files in IDEA while Claude Code is also modifying them, and means `PreToolUse:Edit` hooks may fire on content that differs from what the user sees in the IDE. Workaround: avoid concurrent editing of the same files in both IDEA and Claude Code sessions. See [#43734](https://github.com/anthropics/claude-code/issues/43734).

**Sub-agents do not inherit `bypassPermissions` from the parent session.** When the parent session runs with `bypassPermissions` mode, [spawned teammates via the Agent tool do not inherit this mode](https://github.com/anthropics/claude-code/issues/43736). Each teammate starts in the default permission mode and prompts for confirmations the parent session would have auto-approved. Enforcement hooks that depend on the session being non-interactive throughout a multi-agent workflow will be bypassed inconsistently: the parent approves silently, sub-agents prompt. Workaround: explicitly set `mode: "bypassPermissions"` in each Agent tool call, or set `initialPermissionMode: "bypassPermissions"` in the project's `settings.json`. See [#43736](https://github.com/anthropics/claude-code/issues/43736).

**Scheduled remote triggers return HTTP 200 but silently never execute.** Scheduled remote triggers created via the CLI [accept the creation request (HTTP 200) but never execute their scheduled runs](https://github.com/anthropics/claude-code/issues/43741). No error is returned at creation time or when the scheduled time passes. Automation workflows that rely on scheduled triggers receive no output and no failure signal, making the failure invisible to monitoring and to any Stop hooks that would otherwise fire. Workaround: use cron or launchd as the scheduling layer instead of Claude Code's built-in trigger system. See [#43741](https://github.com/anthropics/claude-code/issues/43741).

**Custom marketplace plugins show incorrect install state in Discover tab; updates don't propagate from cache.** Plugins from custom marketplaces [show as `✔ enabled` in the Installed tab but appear as not installed in the Discover tab](https://github.com/anthropics/claude-code/issues/43745). Updates pushed to the custom marketplace GitHub repo are not pulled from the local cache in `~/.claude/plugins/marketplaces/<name>/`. Hook and enforcement rules shipped in plugin updates are silently not applied until the cache is manually cleared. Related to the stale-cache issue in [#43732](https://github.com/anthropics/claude-code/issues/43732). Workaround: manually delete `~/.claude/plugins/marketplaces/<name>/` to force a re-fetch of the marketplace manifest. See [#43745](https://github.com/anthropics/claude-code/issues/43745).
