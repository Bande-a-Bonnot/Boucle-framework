...
**PreToolUse hook exit codes ignored for subagent tool calls.** When Claude spawns a subagent via the Agent tool, PreToolUse hooks still fire for tool calls inside the subagent, but [the exit code and block decision are silently ignored](https://github.com/anthropics/claude-code/issues/40580). A hook that successfully blocks `git push --force` in the parent session will fire but fail to block the same command in a subagent. This is distinct from settings not propagating ([#37730](https://github.com/anthropics/claude-code/issues/37730)) — here, hooks execute and return a block decision, but the platform discards it. This is the most severe hook limitation for multi-agent workflows: enforcement is parent-session-only. There is no workaround. See [#40580](https://github.com/anthropics/claude-code/issues/40580).
