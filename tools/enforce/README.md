...
| All hooks silently disabled | `CLAUDE_CODE_SIMPLE` env var set, or `IS_DEMO=1`, silently disables every hook, MCP tool, and CLAUDE.md loading (v2.1.50) | `safety-check` warns when these env vars are detected. Unset them before running Claude Code. |
