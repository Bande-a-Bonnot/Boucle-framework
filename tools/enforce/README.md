...
**Symlink-target matching undocumented for Read/Edit permission rules.** When `Read` or `Edit` permission rules use absolute paths (`//path`), it is [undocumented how symlinked paths are evaluated](https://github.com/anthropics/claude-code/issues/41793) — whether rules match the symlink path, the resolved target, or both. This creates uncertainty for enforcement: a deny rule on `/etc/passwd` might not match if the model reads via a symlink like `/tmp/link-to-passwd`. Hook-based enforcement using `file-guard` matches on the tool input path (what the model requested), not the resolved target. For sensitive path protection, use both the symlink path and the resolved target in deny rules. See [#41793](https://github.com/anthropics/claude-code/issues/41793).
