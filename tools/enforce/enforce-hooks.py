...
FILE_MUTATION_TOOL_LABEL = ', '.join(FILE_MUTATION_TOOLS)
BASH_COMMAND_TOKEN_PATTERNS = {
    'cp', 'mv', 'rm', 'tee', 'touch', 'mkdir', 'unlink', 'chmod', 'chown', 'sudo',
}
...
REQUIRE_PRIOR_PATTERNS = [
    ...
    # "Read MEMORY.md before editing any file" / "Read project.md before any Write"
    # Captures specific filename (group 1) and target action (group 2)
    re.compile(
        r'(?:always\s+)?read\s+'
        r'(?:the\s+)?'
        r'([A-Za-z0-9_./\-]+\.(?:md|txt|json|yaml|yml|toml|cfg|ini|conf))'
        r'\s+before\s+'
        r'(?:any\s+)?'
        r'(editing|modifying|changing|updating|writing|write|edit|modification|file)',
        re.IGNORECASE
    ),
...
def extract_command_patterns(text):
    ...
    lower = text.lower()
    force_push_specific = any(
        phrase in lower
        for phrase in ('force push', 'push --force', 'push -f', 'git push --force', 'git push -f')
    )
    ...
    for key in sorted(known_commands, key=len, reverse=True):
        if key == 'git push' and force_push_specific:
            continue
...
def _bash_pattern_matches(pattern, command):
    """Return true when a bash-guard pattern matches a command.

    Most multi-token patterns intentionally stay substring-based because they
    describe shell fragments such as "git push" or "rm -rf". Single command
    names need token-aware matching so a rule for "rm" does not fire on a word
    like "term".
    """
    pat = pattern.strip()
    if not pat:
        return False
    if pat.lower() in BASH_COMMAND_TOKEN_PATTERNS:
        token = re.escape(pat)
        return bool(re.search(rf'(?<![A-Za-z0-9_./-]){token}(?![A-Za-z0-9_./-])', command, re.IGNORECASE))
    return pat.lower() in command.lower()


def _bash_pattern_is_command_token(pattern):
    return pattern.strip().lower() in BASH_COMMAND_TOKEN_PATTERNS
    ...
    for idx, pattern in enumerate(REQUIRE_PRIOR_PATTERNS):
        ...
        if m:
            ...
            if idx == 1:
                ...
                # Check if the directive references a specific file to read first
                # e.g. "Read MEMORY.md before editing any file"
                file_match = re.search(
                    r'\bread\s+(?:the\s+)?([A-Za-z0-9_./\-]+\.(?:md|txt|json|yaml|yml|toml|cfg|ini|conf))\b',
                    clean, re.IGNORECASE)
                if file_match:
                    required_file = file_match.group(1)
                    target = _to_tool(target_raw)
                    ...
                    return Directive(
                        text=stripped,
                        hook_type='require-prior-read-file',
                        patterns=[required_file, target_tools],
                        description=f"Read {required_file} before {target_tools}",
                        line_num=line_num,
                        severity=severity,
                    )
                required = _to_tool(required_raw)
...
REQUIRE_PRIOR_READ_FILE_TEMPLATE = '''#!/usr/bin/env bash
# enforce: {directive}
# Generated by enforce-hooks from CLAUDE.md
# NOTE: Requires session-log hook to be installed for tracking.
# Checks that a specific file was Read before allowing Edit/Write.
INPUT=$(cat)
TOOL=$(echo "$INPUT" | jq -r '.tool_name')
case "$TOOL" in {target_tools}) ;; *) exit 0 ;; esac
TODAY=$(date -u +%Y-%m-%d)
LOG="$HOME/.claude/session-logs/$TODAY.jsonl"
if [ -f "$LOG" ]; then
  if grep '"tool":"Read"' "$LOG" 2>/dev/null | grep -q '{required_file}'; then
    exit 0
  fi
fi
echo '{{"hookSpecificOutput": {{"permissionDecision": "deny", "permissionDecisionReason": "Read {required_file} first. (CLAUDE.md: {short})"}}}}'
exit 0
'''

REQUIRE_PRIOR_READ_FILE_WARN_TEMPLATE = '''#!/usr/bin/env bash
...
echo "⚠ enforce-hooks WARN: Read {required_file} first. (CLAUDE.md: {short})" >&2
exit 0
'''
...
def generate_bash_guard_checks(patterns, short_directive, severity='block'):
    ...
    for pat in patterns:
        ...
        short = short_directive.replace('"', '\\"').replace("'", "'\\''")
        if _bash_pattern_is_command_token(pat):
            regex = re.escape(pat)
            check_expr = f'[[ "$CMD" =~ (^|[^[:alnum:]_./-]){regex}([^[:alnum:]_./-]|$) ]]'
        else:
            check_expr = f'[[ "$CMD" == *"{escaped}"* ]]'
        if severity == 'warn':
            lines.append(
                f'{check_expr} && '
...
def generate_hook(directive):
    ...
    elif directive.hook_type == 'require-prior-read-file':
        if len(directive.patterns) >= 2:
            required_file = directive.patterns[0]
            target_tools = directive.patterns[1]
            if sev == 'warn':
                return REQUIRE_PRIOR_READ_FILE_WARN_TEMPLATE.format(
                    directive=short,
                    target_tools=target_tools,
                    required_file=required_file,
                    short=_truncate_short(directive.text),
                )
            return REQUIRE_PRIOR_READ_FILE_TEMPLATE.format(
                directive=short,
                target_tools=target_tools,
                required_file=required_file,
                short=_truncate_short(directive.text),
...
def _check_single(directive, tool_name, tool_input):
    ...
    if directive.hook_type == 'scoped-content-guard':
        ...
        if not cmd:
            return None
        for pat in directive.patterns:
            if _bash_pattern_matches(pat, cmd):
                return {"decision": "block",
        ...
        if len(directive.patterns) >= 2:
            required_file = directive.patterns[0]
            target_tools = directive.patterns[1]
            # Check if the current tool is one of the targets
            if tool_name not in target_tools.split('|'):
                return None
                ...
                try:
                    for line in session_log.read_text().splitlines():
                        try:
                            ...
                            if entry.get('tool') == 'Read' and required_file in entry.get('detail', ''):
                                found = True
                                break
                        except json.JSONDecodeError:
                            pass
            ...
            if not found:
                return {"decision": "block",
                        "reason": f"Read {required_file} first. (CLAUDE.md: \"{short}\")"}
...
def verify_hooks(hooks_dir='.claude/hooks', settings_path='.claude/settings.json'):
    ...
    def is_plugin_helper(hook_path):
        """Ignore plugin-mode helper code when the registered hook is its wrapper."""
        if hook_path.name != 'enforce-hooks.py':
            return False
        for cmd in registered_commands:
            if 'enforce-pretooluse.sh' not in cmd:
                continue
            for part in cmd.split():
                if Path(part).name != 'enforce-pretooluse.sh':
                    continue
                try:
                    if Path(part).resolve().parent == hook_path.resolve().parent:
                        return True
                except OSError:
                    if Path(part).parent == hook_path.parent:
                        return True
        return False
    ...
    hook_files = sorted(
        ...
        if f.is_file() and not f.name.startswith('.') and not is_plugin_helper(f)
    )
...
def run_tests():
    ...
    check("hook bash case-insensitive matching", hook is not None and 'shopt -s nocasematch' in hook, True)

    d = Directive(
        text="Never run rm",
        hook_type='bash-guard',
        patterns=['rm'],
        description="Block commands: rm",
        line_num=5,
    )
    hook = generate_hook(d)
    check("hook gen bash token-aware rm", hook is not None and '[^[:alnum:]_./-])rm' in hook, True)
    if hook:
        ...
        try:
            ...
            proc = subprocess.run(
                [hook_path],
                input=json.dumps({"tool_name": "Bash", "tool_input": {"command": "RM notes.md"}}),
                capture_output=True,
                text=True,
                check=True,
            )
            check("hook bash token case-insensitive rm", '"permissionDecision": "deny"' in proc.stdout, True)
            proc = subprocess.run(
                [hook_path],
                input=json.dumps({"tool_name": "Bash", "tool_input": {"command": "echo term notes.md"}}),
                capture_output=True,
                text=True,
                check=True,
            )
            check("hook bash token avoids term", proc.stdout.strip(), "")
        finally:
            os.unlink(hook_path)

    d = Directive(
        text="Never run ALTER, DROP, TRUNCATE, INSERT, UPDATE, or DELETE",
        hook_type='bash-guard',
        patterns=['ALTER', 'DROP', 'TRUNCATE', 'INSERT', 'UPDATE', 'DELETE'],
        description="Block commands: ALTER, DROP, TRUNCATE, INSERT, UPDATE, DELETE",
        line_num=6,
    )
    hook = generate_hook(d)
    if hook:
        ...
        try:
            ...
            proc = subprocess.run(
                [hook_path],
                input=json.dumps({"tool_name": "Bash", "tool_input": {"command": "psql -c 'drop table users'"}}),
                capture_output=True,
                text=True,
                check=True,
            )
            check("hook bash SQL case-insensitive drop", '"permissionDecision": "deny"' in proc.stdout, True)
        finally:
            os.unlink(hook_path)
    ...
    check("extract cmd push force not normal push", 'git push' in pats, False)
...
"""
    ...
    d = Directive("Never run rm", "bash-guard", ["rm"], "block rm", 5)
    r = evaluate_tool_call([d], "Bash", {"command": "rm notes.md"})
    check("eval: blocks rm command token", r["decision"], "block")
    r = evaluate_tool_call([d], "Bash", {"command": "echo term notes.md"})
    check("eval: rm token does not match term", r["decision"], "allow")

    d = Directive("Never run touch or mkdir", "bash-guard", ["touch", "mkdir"], "block fs creation", 6)
    r = evaluate_tool_call([d], "Bash", {"command": "touch notes.md"})
    check("eval: blocks touch command token", r["decision"], "block")
    r = evaluate_tool_call([d], "Bash", {"command": "mkdir audit-output"})
    check("eval: blocks mkdir command token", r["decision"], "block")
...
"""
    ...
    # --- Read specific file before editing (reading-order enforcement) ---

    d = classify_directive("Read MEMORY.md before editing any file", 97)
    check("read-file-order detected", d is not None and d.hook_type == 'require-prior-read-file', True)
    check("read-file-order file", d is not None and d.patterns[0] == 'MEMORY.md', True)
    check("read-file-order targets", d is not None and 'Edit' in d.patterns[1], True)

    d = classify_directive("Always read project.md before any modification", 98)
    check("read-file-order project.md", d is not None and d.hook_type == 'require-prior-read-file', True)
    check("read-file-order project file", d is not None and d.patterns[0] == 'project.md', True)

    d = classify_directive("Read CLAUDE.md before writing any file", 99)
    check("read-file-order CLAUDE.md", d is not None and d.hook_type == 'require-prior-read-file', True)
    check("read-file-order CLAUDE file", d is not None and d.patterns[0] == 'CLAUDE.md', True)

    d = classify_directive("Read rules.yaml before editing", 100)
    check("read-file-order yaml", d is not None and d.hook_type == 'require-prior-read-file', True)
    check("read-file-order yaml file", d is not None and d.patterns[0] == 'rules.yaml', True)

    d = classify_directive("Read config/settings.toml before any modification", 101)
    check("read-file-order nested path", d is not None and d.hook_type == 'require-prior-read-file', True)
    check("read-file-order nested file", d is not None and d.patterns[0] == 'config/settings.toml', True)

    # Verify hook generation for require-prior-read-file
    d = Directive("Read MEMORY.md before editing", 'require-prior-read-file',
                  ['MEMORY.md', 'Write|Edit|MultiEdit'], "Read MEMORY.md first", severity='block')
    script = generate_hook(d)
    check("read-file-order generates hook", script is not None, True)
    check("read-file-order hook has MEMORY.md", script is not None and 'MEMORY.md' in script, True)
    check("read-file-order hook has deny", script is not None and 'deny' in script, True)
    check("read-file-order hook checks Read tool", script is not None and '"tool":"Read"' in script, True)

    d = Directive("Read MEMORY.md before editing", 'require-prior-read-file',
                  ['MEMORY.md', 'Write|Edit|MultiEdit'], "Read MEMORY.md first", severity='warn')
    script = generate_hook(d)
    check("read-file-order warn generates hook", script is not None, True)
    check("read-file-order warn has WARN", script is not None and 'WARN' in script, True)
    check("read-file-order warn no deny", script is not None and 'deny' not in script, True)
    ...
    # Plugin mode installs a wrapper plus a helper copy. Verify should check the
    # registered wrapper, not treat the helper source file as an unregistered
    # executable hook with fixture strings that look like wrong JSON fields.
    plugin_project = os.path.join(td, 'plugin-project')
    os.makedirs(plugin_project)
    plugin_claude = os.path.join(plugin_project, 'CLAUDE.md')
    with open(plugin_claude, 'w') as f:
        f.write('## Read-only audit mode @enforced\n')
        f.write('- Never modify any files\n')
        f.write('- Never run git push\n')
    install_plugin(
        plugin_claude,
        os.path.join(plugin_project, '.claude', 'hooks'),
        os.path.join(plugin_project, '.claude', 'settings.json'),
    )
    plugin_result = verify_hooks(
        os.path.join(plugin_project, '.claude', 'hooks'),
        os.path.join(plugin_project, '.claude', 'settings.json'),
    )
    check("verify: plugin mode scans wrapper only", plugin_result['summary']['total'], 1)
    check("verify: plugin mode has no errors", plugin_result['summary']['errors'], 0)
    wrapper = os.path.join(plugin_project, '.claude', 'hooks', 'enforce-pretooluse.sh')
    ...
    plugin_decision = json.loads(proc.stdout).get('decision') if proc.returncode == 0 else ''
    check("verify: plugin wrapper still blocks writes", plugin_decision, 'block')
