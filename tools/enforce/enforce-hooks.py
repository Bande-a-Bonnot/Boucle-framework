...
FILE_GUARD_PATTERNS = [
    # "Never modify/edit/write/touch/change .env files"
    re.compile(
        r'(?:never|don\'?t|do\s+not|avoid|prohibited?)\s+'
        r'(?:modify|edit|write\s+to|touch|change|alter|update|overwrite|delete|remove)\s+'
        r'(.+)',
        re.IGNORECASE
    ),
...
BRANCH_GUARD_PATTERNS = [
    # "Never commit/push/merge to main" or "Don't commit to main or production"
    re.compile(
...
REQUIRE_PRIOR_PATTERNS = [
    ...
    # "Search locally before using web search"
    re.compile(
        r'(?:search|check|look)\s+(?:locally|the\s+codebase|existing\s+code)\s+'
        r'before\s+(?:using?\s+)?(.+)',
        re.IGNORECASE
    ),
]
...
TOOL_BLOCK_PATTERNS = [
    ...
    re.compile(
        ...
        r'(?:use|call|invoke)\s+'
        r'(?:the\s+)?'
        r'((?:Web(?:Search|Fetch)|NotebookEdit|Agent|Bash)\b.+)',
        re.IGNORECASE
    ),
]
...
def extract_command_patterns(text):
    ...
    dangerous = [
        ...
        'reset --hard', 'clean -fd', 'clean -f', 'checkout .', 'restore .',
...
def generate_hook(directive):
    ...
    if directive.hook_type == 'file-guard':
        ...
        return FILE_GUARD_TEMPLATE.format(directive=short, checks=checks)
