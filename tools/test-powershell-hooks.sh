...
if grep -q 'install.ps1 backup.*Snapshot before updating Claude Code' "$SCRIPT_DIR/install.ps1" &&
   grep -q 'restore \[file\].*without file, use the most recent backup' "$SCRIPT_DIR/install.ps1" &&
   grep -q 'install.ps1 backup list.*Inspect backups and choose the filename to restore' "$SCRIPT_DIR/install.ps1" &&
   grep -q 'install.ps1 restore settings\.20260101_120000\.json.*Restore a named backup from backup list' "$SCRIPT_DIR/install.ps1" &&
   grep -q 'install.ps1 restore.*only if you meant it' "$SCRIPT_DIR/install.ps1"; then
