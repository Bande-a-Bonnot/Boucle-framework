...
echo "  git commit --no-verify (let pre-commit hooks run)"
...
echo "  git checkout HEAD -- . (overwrites files from ref)"
echo "  git restore <path>     (without --staged, discards changes)"
