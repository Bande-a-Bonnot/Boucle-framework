...
if ($Hooks -and $Hooks.Count -gt 0 -and ($Hooks[0] -eq 'help' -or $Hooks[0] -eq '--help' -or $Hooks[0] -eq '-h')) {
    ...
    Write-Host "  install.ps1 backup list            # Inspect backups and choose the filename to restore"
    Write-Host "  install.ps1 restore settings.20260101_120000.json # Restore a named backup from backup list"
    Write-Host "  install.ps1 restore                # Restore the most recent backup only if you meant it"
