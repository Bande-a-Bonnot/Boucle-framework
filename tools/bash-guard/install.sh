...
echo "  - Library injection (LD_PRELOAD, LD_LIBRARY_PATH)"
echo "  - Wrapper bypass (timeout/nohup/strace hiding dangerous ops)"
echo "  - Credential file copy (cp/mv/scp .ssh/, .aws/, .gnupg/)"
echo "  - macOS Keychain access (security find-generic-password)"
echo "  - Scheduled tasks (crontab, launchctl)"
echo "  - Service management (systemctl, service start/stop)"
echo "  - SSH key generation (ssh-keygen, ssh-add)"
echo "  - git filter-branch (history rewriting)"
echo ""
