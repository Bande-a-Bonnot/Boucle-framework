...
assert_blocked "ssh-keygen bare" "ssh-keygen"
assert_blocked "ssh-keygen -p passphrase" "ssh-keygen -p -f ~/.ssh/id_rsa"
assert_blocked "ssh-keygen glued -R (not a flag)" "ssh-keygen -Rhermes"
...
# known_hosts management is exempt: -R (remove host), -F (find host)
assert_allowed "ssh-keygen -R host" "ssh-keygen -R hermes"
assert_allowed "ssh-keygen -R ip" "ssh-keygen -R 100.64.1.2"
assert_allowed "ssh-keygen -F host" "ssh-keygen -F github.com"
assert_allowed "ssh-keygen -R after chain" "echo done && ssh-keygen -R hermes"
# Bypass guard: a safe -R must NOT mask an unsafe keygen elsewhere in the chain
assert_blocked "ssh-keygen safe-then-unsafe" "ssh-keygen -R hermes && ssh-keygen -t rsa"
assert_blocked "ssh-keygen unsafe-then-safe" "ssh-keygen -t rsa; ssh-keygen -R hermes"
