...
HOOK_NAME="worktree-guard"
HOOK_URL="https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/worktree-guard/hook.sh"
...
# Register in settings.json under PreToolUse with ExitWorktree matcher
...
for h in pre:
    ...
    if '$HOOK_NAME' in cmd:
        if '$HOOK_PATH' in cmd:
            found = True
            # Ensure it has the matcher
            h['matcher'] = 'ExitWorktree'
            if 'hooks' not in h:
                cleaned.append({'hooks': [{'type': 'command', 'command': '$HOOK_PATH'}], 'matcher': 'ExitWorktree'})
            else:
                cleaned.append(h)
        # else: old path, skip
    else:
...
" 2>/dev/null || {
    ...
    echo '  hooks.PreToolUse: [{"hooks": [{"type": "command", "command": "'$HOOK_PATH'"}], "matcher": "ExitWorktree"}]'
  }
else
...
echo ""
echo "Done! worktree-guard is now active."
echo ""
echo "When you exit a worktree (claude -w), the hook checks for:"
echo "  - Uncommitted changes (staged/unstaged)"
echo "  - Untracked files"
echo "  - Unmerged commits (not in main/master)"
echo "  - Unpushed commits (if upstream is set)"
echo ""
echo "If any check fails, exit is blocked until you commit, push, or merge."
echo ""
echo "To customize, create .worktree-guard:"
echo "  allow: uncommitted    # skip uncommitted check"
echo "  allow: untracked      # skip untracked files check"
echo "  allow: unmerged       # skip unmerged commits check"
echo "  allow: unpushed       # skip unpushed commits check"
echo "  base: develop         # override base branch"
echo ""
echo "Config:"
echo "  Hook: $HOOK_PATH"
echo "  Disable: WORKTREE_GUARD_DISABLED=1"
echo "  Debug: WORKTREE_GUARD_LOG=1"
