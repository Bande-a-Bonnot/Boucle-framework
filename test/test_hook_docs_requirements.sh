...
docs = {
    repo / "README.md": [
        ...
        "Installing adds managed hook files under `~/.claude/<hook-name>/`",
        "`hook.ps1` on native Windows",
        ...
        "representative hook payloads to installed `PreToolUse` hooks",
        "Other hook events are inventoried and reported, but skipped for payload verification",
        ...
        "tools/install.sh | bash -s -- recommended curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.sh | bash -s -- doctor curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.sh | bash -s -- verify",
        "tools/install.sh | bash -s -- all curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.sh | bash -s -- doctor curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.sh | bash -s -- verify",
        "tools/install.ps1) } all\" iex \"& { $(irm https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.ps1) } doctor\" iex \"& { $(irm https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.ps1) } verify",
        ...
        "safety-check` runs a bounded `claude --version` probe",
        "Claude Code version: version probe hangs",
        "<a id=\"test-hook\"></a>",
    ],
    repo / "tools" / "README.md": [
        ...
        "avoid repo-relative commands such as `python3 scripts/guard.py`",
        "prefer absolute or verified project-rooted paths",
        ...
        "install.ps1 restore settings.20260101_120000.json",
        "Replace `settings.20260101_120000.json` with the backup name",
        "use bare `restore` only when the most recent backup is the exact",
        'if ($root) { Set-Location $root }',
        "--- End Safety Summary ---",
        "safety-check --verify --summary-only",
        "tools/install.sh | bash curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.sh | bash -s -- doctor curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.sh | bash -s -- verify",
        "tools/install.sh | bash -s -- recommended curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.sh | bash -s -- doctor curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.sh | bash -s -- verify",
        "tools/install.sh | bash -s -- read-once git-safe file-guard curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.sh | bash -s -- doctor curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.sh | bash -s -- verify",
    ...
    repo / "tools" / "safety-check" / "README.md": [
        ...
        "representative test payloads to installed `PreToolUse` hooks",
        "Other hook events are inventoried and reported, but skipped for payload verification",
        ...
        "cd \"$repo_root\" curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/safety-check/check.sh | bash -s -- --verify --summary-only",
        "cd \"$repo_root\" curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/safety-check/check.sh | bash -s -- --verify --strict",
        "Without `--strict`, verification findings are report-only",
        ...
        "install.sh | bash -s -- backup list",
        "install.sh | bash -s -- restore settings.20260101_120000.json",
        "install.ps1) } backup list",
        "install.ps1) } restore settings.20260101_120000.json",
    ...
    repo / "tools" / "safety-check" / "TRIAGE.md": [
        ...
        "Claude Code 2.1.233-2.1.235 can OOM at startup with `sandbox.enabled: true`",
        "remove the sandbox block or set `enabled: false` before starting long or unattended sessions",
        "github.com/anthropics/claude-code/issues/88029",
    ...
    repo / "tools" / "safety-check" / "SUPPORT_EXAMPLES.md": [
        ...
        "does not inspect your real Claude Code settings",
    ...
    repo / "tools" / "safety-check" / "QUICKSTART.md": [
        ...
        "curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.sh | bash -s -- recommended curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.sh | bash -s -- doctor curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.sh | bash -s -- verify",
        "curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.sh | bash -s -- all curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.sh | bash -s -- doctor curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/install.sh | bash -s -- verify",
        "The installer verifier sends representative payloads to the installed hooks",
        "representative Claude-style JSON payloads to installed `PreToolUse` hooks",
        "Other hook events are inventoried and reported, but skipped for payload verification",
        "does not execute the dangerous shell or git commands",
        ...
        "tools/install.ps1) } backup list",
        "tools/install.ps1) } restore settings.20260101_120000.json",
        "cd \"$repo_root\" curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/safety-check/check.sh | bash",
        "cd \"$repo_root\" curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/safety-check/check.sh | bash -s -- --verify",
        "cd \"$repo_root\" curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/safety-check/check.sh | bash -s -- --verify --strict",
        "cd \"$repo_root\" curl -fsSL https://raw.githubusercontent.com/Bande-a-Bonnot/Boucle-framework/main/tools/safety-check/check.sh | bash -s -- --verify --summary-only",
        "cd \"$repo_root\" unset IS_DEMO CLAUDE_CODE_SIMPLE",
    ...
    repo / "tools" / "safety-check" / "UPDATE_CHECKLIST.md": [
        ...
        "Capture the current bounded summary before changing anything",
        "stuck `claude --version` cannot hang the baseline capture",
        "Claude Code version: version probe hangs",
        "Command boundary: this downloads `tools/safety-check/check.sh` from GitHub raw",
        "runs it locally on your current project and Claude Code settings",
        "does not upload your `settings.json`, hook files, shell history, repository contents, session logs, or safety summary output",
        ...
        "--verify --summary-only",
        "record the bounded safety summary and the native verifier count",
        "install.ps1) } check --verify --summary-only",
    ...
    repo / "tools" / "safety-check" / "TEAM_HANDOFF.md": [
        ...
        "Pre-update baseline: summary/version captured / not captured / not an update issue",
        "Staged diff and destination reviewed separately: yes / no / not applicable",
        "destination branch, and remote target separately before approval",
    ...
    repo / "docs" / "index.html": [
        ...
        "Installing adds managed hook files under <code>~/.claude/&lt;hook-name&gt;/</code>",
        "<code>hook.ps1</code> on native Windows",
        "The installer does not upload your settings, hook files, shell history, repository contents, or safety summary output",
        ...
        "tools/install.sh | bash -s -- restore settings.20260101_120000.json",
        "tools/install.ps1) } restore settings.20260101_120000.json",
        "Replace <code>settings.20260101_120000.json</code> with the backup name you selected",
        "Verify: not run",
        "0 payload checks",
        "FAIL-OPEN",
    ...
    repo / "docs" / "llms.txt": [
        ...
        "whether a fresh Claude Code session was started from the verified root",
        "After Claude Code, hooks, settings, MCP servers, connectors, or plugins change",
        "capture `--verify --summary-only` from the same project root first",
        "run strict verification and start a fresh Claude Code session from that verified root",
        "For resumed or compacted Claude Code sessions",
        "Verify from disk, start a fresh session, and run one harmless canary",
        "For MCP/plugin changes, treat new tools, missing tools, changed schemas",
        "connector crossing into send/delete/publish behavior as a renewed approval event",
        "If the next intended action is commit, push, or public report",
        "staged diff, destination branch, and remote target separately",
        "summary proves only the hook boundary",
        "https://framework.boucle.sh/limitations.json",
        "https://framework.boucle.sh/limitations-feed.xml",
    ...
    repo / "docs" / "recipes.html": [
        ...
        "Run <code>backup list</code> first, inspect the available snapshots, and restore the named backup you intend to use.",
        "Use bare <code>restore</code> only when the most recent backup is the exact snapshot you want back.",
        ...
        "sends representative Claude-style <code>PreToolUse</code> payloads to installed <code>PreToolUse</code> hooks",
        "other hook events are inventoried and reported, but skipped for payload verification",
        "Skipped non-<code>PreToolUse</code> events remain inventory unless no <code>PreToolUse</code> payload checks ran.",
        "representative <code>PreToolUse</code> payload checks did not find a configured <code>PreToolUse</code> hook that failed open",
